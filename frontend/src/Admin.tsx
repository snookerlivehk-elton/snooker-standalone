        ...
        <div className="w-full flex justify-end mb-4 gap-2">
          <button
            onClick={() => {
              ...
              const url = `${base}/admin/members${tok ? `?token=${encodeURIComponent(tok)}&v=members` : '?v=members'}`;
          ...
          <button
            onClick={() => {
              const tok = localStorage.getItem('adminToken') || '';
              const base = getFrontendBase();
              const url = `${base}/admin/matches${tok ? `?token=${encodeURIComponent(tok)}&v=matches` : '?v=matches'}`;
              window.location.href = url;
            }}
          ...
          <button
            onClick={() => {
              const tok = localStorage.getItem('adminToken') || '';
              const base = getFrontendBase();
              const url = `${base}/admin/regions${tok ? `?token=${encodeURIComponent(tok)}&v=regions` : '?v=regions'}`;
              window.location.href = url;
            }}
        ...
        <div className="bg-gray-800 rounded-lg shadow-lg p-6 mb-8">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              onClick={() => {
                const base = getFrontendBase();
                const url = `${base}/room/default?simple=true&enableSocket=true&socketUrl=${encodeURIComponent(SOCKET_URL)}&apiUrl=${encodeURIComponent(API_URL)}&socketPath=${encodeURIComponent(SOCKET_PATH)}`;
            ...
            <button
              onClick={() => {
                const base = getFrontendBase();
                const url = `${base}/room/default/overlay?simple=true&enableSocket=true&socketUrl=${encodeURIComponent(SOCKET_URL)}&socketPath=${encodeURIComponent(SOCKET_PATH)}`;
                navigator.clipboard.writeText(url).then(() => {
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      ...
                      const base = getFrontendBase();
                      const url = `${base}/room/${displayId}/setup${params}`;
                      navigator.clipboard.writeText(url).then(() => {
                  ...
                  <button
                    onClick={() => {
                      const params = `?enableSocket=1&socketUrl=${encodeURIComponent(SOCKET_URL)}&apiUrl=${encodeURIComponent(API_URL)}`;
                      const base = getFrontendBase();
                      const url = `${base}/room/${displayId}/live${params}`;
                      navigator.clipboard.writeText(url).then(() => {
                        alert(`已複製 Live 連結：\n${url}`);
                      });
                    }}
                  ...
                  <button
                    onClick={() => {
                      const params = `?enableSocket=1&socketUrl=${encodeURIComponent(SOCKET_URL)}&apiUrl=${encodeURIComponent(API_URL)}`;
                      const base = getFrontendBase();
                      const url = `${base}/room/${displayId}/overlay${params}`;
                      navigator.clipboard.writeText(url).then(() => {
